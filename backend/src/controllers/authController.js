...
exports.logout = exports.getProfile = exports.refreshToken = exports.verifyOTP = exports.login = exports.register = void 0;
...
const register = async (req, res) => {
    try {
        ...
        if (!isRecaptchaValid) {
            logger_1.securityLogger.logSuspiciousActivity('Failed reCAPTCHA verification', { email, ip: clientIp });
            res.status(400).json({
                success: false,
        ...
        logger_1.securityLogger.logRegistration(user._id.toString(), email, clientIp);
        res.status(201).json({
            success: true,
...
const login = async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            logger_1.securityLogger.logFailedLogin(email, clientIp, 'User not found');
            res.status(401).json({
                success: false,
        ...
        if (user.lockUntil && user.lockUntil > new Date()) {
            ...
            logger_1.securityLogger.logFailedLogin(email, clientIp, 'Account locked');
            res.status(423).json({
                success: false,
        ...
        if (!isPasswordValid) {
            ...
            const updatedUser = await User_1.default.findById(user._id).select('+loginAttempts +lockUntil');
            const attemptsLeft = 5 - (updatedUser?.loginAttempts || 0);
            logger_1.securityLogger.logFailedLogin(email, clientIp, 'Invalid password');
            res.status(401).json({
                success: false,
...
const verifyOTP = async (req, res) => {
    try {
        ...
        // Verify OTP
        if (user.otp !== otp) {
            logger_1.securityLogger.logFailedLogin(email, clientIp, 'Invalid OTP');
            res.status(401).json({
                success: false,
        ...
        logger_1.securityLogger.logSuccessfulLogin(user._id.toString(), email, clientIp);
        const userData = {
            userId: user._id,
            name: user.name,
            email: user.email,
